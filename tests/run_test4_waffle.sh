...
export TEST_DIR=${ROOT_DIR}/tests/test4
...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    for ((i=1; i<100; i++))
    do
        if ! [ -d "${TEST_DIR}/out_MemLock$i" ]; then
            break
        fi
    done

    # Compile using llvm
    # ${AFL_PATH}/memlock-stack-clang++ -g -fsanitize=address example4.cpp -o example4Waffle
    ${AFL_PATH}/memlock-stack-clang++ example4.cpp -o example4Waffle
    ...
    ${AFL_PATH}/memlock-stack-fuzz -i ${TEST_DIR}/in -o ${TEST_DIR}/out_MemLock$i -m none -d -t 5000 -- ${TEST_DIR}/example4Waffle @@
    echo "Execute the afl-showmap..."
    ss=$(echo 0 | ${AFL_PATH}/afl-showmap -m none -q -o .example4Waffle ${TEST_DIR}/example4Waffle) || true
fi
