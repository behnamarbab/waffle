...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    ${ROOT_DIR}/tool/MemLock/memlock-stack-clang -g -O0 -fsanitize=address example3.c -o example3MemLock
    ...
    for ((i=1; i<100; i++))
    do
        if ! [ -d "${ROOT_DIR}/tests/test3/out_MemLock$i" ]; then
            break
        fi
    done
    export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=1:abort_on_error=1:symbolize=0:detect_leaks=0
    ${ROOT_DIR}/tool/MemLock/memlock-stack-fuzz -i ${ROOT_DIR}/tests/test3/in -o ${ROOT_DIR}/tests/test3/out_MemLock$i -m none -d -t 5000 -- ${ROOT_DIR}/tests/test3/example3MemLock @@
fi
